...
namespace gps {
	...
	gps::BoundingBox Model3D::GetBoundingBox() const {
		BoundingBox bbox;
		bbox.minCoords = glm::vec3(FLT_MAX);
		bbox.maxCoords = glm::vec3(FLT_MIN);

		for (const auto& mesh : meshes) {
			for (const auto& vertex : mesh.vertices) {
				bbox.minCoords.x = std::min(bbox.minCoords.x, vertex.Position.x);
				bbox.minCoords.y = std::min(bbox.minCoords.y, vertex.Position.y);
				bbox.minCoords.z = std::min(bbox.minCoords.z, vertex.Position.z);

				bbox.maxCoords.x = std::max(bbox.maxCoords.x, vertex.Position.x);
				bbox.maxCoords.y = std::max(bbox.maxCoords.y, vertex.Position.y);
				bbox.maxCoords.z = std::max(bbox.maxCoords.z, vertex.Position.z);
			}
		}
